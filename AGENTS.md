...
This is a local-first Kanban app: a Rust core, a Tauri shell, and a TypeScript webview, with everything in one SQLite file. It is a Cargo workspace of two crates. The same TypeScript is also published to GitHub Pages as a browser-only build, with `localStorage` in SQLite's place (ADR 0042).
...
- `crates/core/` is `ekanban-core`: the board model, SQLite, backups, file locations. **It depends on no UI toolkit** — `tauri` must not reach it — and `script/check-core-independence` fails CI if one gets pulled in. See `docs/DESIGN.md`「層の分け方」 for why.
  - `crates/core/src/model.rs` defines the **shape** of a board — `Board`, `Column`, `Card` — and `validate()`, which checks a board handed in from the webview before it is stored. **It decides nothing about the board**: moving, reordering, undo and archiving are `web/src/model/board.ts` (ADR 0039, ADR 0040).
  - `crates/core/src/store.rs` is the one door to wherever a board is kept: `Store` over SQLite (`db`, the `sqlite` feature). **A board decision written into `Store` is a second implementation** — it checks what it is asked to persist (`Board::validate`) and never decides again (ADR 0040).
...
- `crates/app/` is `ekanban-app`, the Tauri binary (`ekanban`). `commands.rs` holds the commands described in `docs/DESIGN.md`「コマンドとイベント」 — **only what a web app would put on the server**: reading and writing the store, numbering and deleting boards, the settings we remember, and what the OS is asked to do (ADR 0039) — and **knows nothing about `tauri`**, so its tests run without a window. `ipc.rs` is nothing but `#[tauri::command]` wrappers over those functions; put no judgement there. `crates/app/tests/commands.rs` calls every command and checks what reached SQLite.
  - `menu.rs` **builds the OS menu bar out of the structure the webview sends** (`ipc::set_menu`, ADR 0043): what is on the bar is `web/src/shell/menu.ts`, and Rust turns it into a `tauri::menu::Menu` and routes the clicks. Keep the `Action` ids and the shape in `Section`/`Item` in step with the generated types — `make types-check` is what catches a drift. What a menu item does is split in two: `AppAction` goes to the webview over `app:action` (everything that touches the board, a draft, or the display state — the screen is what knows), `WindowAction` is done in Rust (close, quit, fullscreen).
...
`make check` runs all seven; `make e2e` is separate because it builds browsers. `make web-demo` and `make e2e-demo` build and drive the browser-only build — `npm` alone now, no Rust (ADR 0042) — and are separate for the same reason. `.github/workflows/pages.yml` builds it on pull requests as well as on `main`, so a broken browser build is caught before it is published.
