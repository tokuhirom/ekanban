...
This is a local-first Kanban app: a Rust core, a Tauri shell, and a TypeScript webview, with everything in one SQLite file. It is a Cargo workspace. The same core is also built for `wasm32-unknown-unknown` and published to GitHub Pages as a browser-only build (ADR 0035).
...
- `crates/core/` is `ekanban-core`: the board model, SQLite, backups, file locations. **It depends on no UI toolkit** — `tauri` must not reach it — and `script/check-core-independence` fails CI if one gets pulled in. See `docs/DESIGN.md`「層の分け方」 for why.
  ...
  - `crates/core/src/store.rs` is the one door to wherever a board is kept: `Store` over SQLite (`db`, the `sqlite` feature) for the app and `JsonStore` for the browser build (ADR 0036). **Only "how it is put away" is doubled** — numbering, reordering, undo and archiving stay in `model.rs`, and a board decision written into `Store` is a second implementation.
...
- `crates/app/` is `ekanban-app`, the Tauri binary (`ekanban`). `commands.rs` holds the commands described in `docs/DESIGN.md`「コマンドとイベント」 — **only what a web app would put on the server**: reading and writing the store, numbering and deleting boards, the settings we remember, and what the OS is asked to do (ADR 0039) — and **knows nothing about `tauri`**, so the browser build can reuse it. `ipc.rs` is nothing but `#[tauri::command]` wrappers over those functions; put no judgement there. `crates/app/tests/commands.rs` calls every command and checks what reached SQLite.
  ...
  - Choosing where a file goes is the OS dialog (`ipc::choose_save_path`, an `async` command — a sync one runs on the main thread and freezes the window while the dialog is up); writing it is `commands`; saying it was written is a dialog inside the app (ADR 0016). The webview puts those three in order, so the browser build drives the same path with only the choosing replaced.
- `crates/app`'s `shell` feature (on by default) holds everything that knows `tauri`: `ipc.rs`, `run.rs`, `window.rs`, `capture.rs`, the global-shortcut half of `shortcut.rs`, and `menu.rs`'s conversion to a Tauri menu. **Outside it stay `commands`, `dispatch`, `state`, `snapshot`, `error`, and the menu as data** — that is the layer the browser build uses, so `cargo build -p ekanban-app --no-default-features --target wasm32-unknown-unknown` is the real check that the command layer does not know Tauri.
  - `dispatch.rs` is the command-name table, shared by every caller outside the Tauri shell (the browser build). **Never write a second table**: the one place a command can be missing from is the place a screen stops working. Commands whose answer depends on the environment (choosing a save path, revealing a folder, opening a URL, binding a global hotkey) are not in it — `dispatch::invoke` returns `None` and the caller answers them.
- `crates/web/` is `ekanban-web`, the browser-only build (ADR 0035): the same commands on `wasm32-unknown-unknown`, with the board kept as JSON in `localStorage` (`store::JsonStore`). **Do not put SQLite back into it** — compiled for wasm it is 2.1 MB on its own, five times everything else here put together (ADR 0036). **Only environment differences belong here** — where files go, how they leave, how a URL opens. One board decision written here is a second implementation, which is what ADR 0035 exists to prevent. Build it with `make web-demo` (`script/build-web-demo`, needs `wasm-pack`); `make e2e-demo` drives the built artefact in Chromium.
- `web/` is the webview: TypeScript + React + Vite (ADR 0019). `src/ipc/` is the one door to Rust, `src/model/` holds the board decisions that are the webview's own — how typed text is read as a due date and whether one is overdue (`due.ts`, counts included), what a search term matches (`search.ts`), the board itself and its undo stack (`board.ts`), the Markdown a board is written out as (`export.ts`), and the date arithmetic they rest on (`dates.ts`) — with their tests beside them; where a decision lives is settled by asking whether a web app would have written it on the server (ADR 0039), and the store checks what it is asked to persist rather than deciding again (ADR 0040). `src/state/` holds the snapshot and the single path that replaces it (`run()` — nothing else calls `setSnapshot`, and it is what decides whether a failure goes to a field or to a dialog), `src/board/` draws the board, `src/panel/` the card and tag editing panels, `src/shell/` the dialogs, the menu actions (`actions.ts` — each part subscribes to the actions it owns, so "save" reaches the panel holding the draft), the theme, the export/backup flow (`files.ts` — it builds the Markdown itself and asks Rust for the JSON, ADR 0045), and the things a webview must switch off by hand (`harden.ts`). `src/panel/Description.tsx` is a Markdown editor on Lexical that stores a Markdown string (ADR 0033): the webview decides where a URL starts and ends (`panel/links.ts`), but **whether a link may be opened is decided by Rust** (`commands::openable_url`). Drag-and-drop rides on `@dnd-kit/core` (ADR 0022), but **where a card lands is decided by our own `board/dnd.ts` and `board/keyboard.ts`** — the library carries, it does not decide, so it can be dropped. **Compiling `crates/app` needs `web/dist`**, so run `npm --prefix web ci && npm --prefix web run build` before `cargo build`/`cargo test` on a fresh checkout.
- `web/demo/` is the browser build's entry point, and `web/e2e-demo/` its Playwright suite. **Do not re-check board behaviour there** — the same `Board` and the same `ekanban-core` run, so `web/e2e/` already covers it. What only that suite can see is the build's own difference: a board that survives a reload because it lives in `localStorage`.
- `web/e2e/` drives the real board with Playwright and **no process in between** (ADR 0041): `harness.ts` seeds one store per test into `localStorage` (`?store=local`), and `storedBoard(page)` reads it back so a test can check **both** what is on screen and what was saved. The board model it exercises is the real one (`web/src/model/`).
...
- `make e2e` drives the board in **Chromium and WebKit**. Those are not the real webviews, but the three real ones only use two engines — WebView2 is Chromium, WKWebView and WebKitGTK are WebKit — so engine-family differences show up here. What cannot show up is Apple's platform layer (momentum scrolling, rubber-banding, trackpad).

`make check` runs all seven; `make e2e` is separate because it builds browsers. `make web-demo` and `make e2e-demo` build and drive the browser-only build; they are separate too, because they need `wasm-pack` and the `wasm32-unknown-unknown` target. **`Check and test` does not build the wasm** — `.github/workflows/pages.yml` does, on pull requests as well as on `main`, so a broken browser build is caught before it is published.
...
**Never decide platform behaviour from `navigator.userAgent`.** It is a string a webview can change — Playwright's Safari emulation calls itself `Macintosh` on Linux — and getting `secondary` wrong disables a whole key binding. Rust knows the platform at compile time and sends it in `StartupState.platform`. **The browser build is the one exception** (ADR 0035): `wasm32-unknown-unknown` is neither macOS nor Linux, so `web/demo/main.tsx` names the platform and hands it to `start()`. Do not spread that exception into `web/src/`.
...
Screen behaviour is tested with Playwright (`web/e2e/`): drive the board the way a person would, then read the store back with `storedBoard(page)` and assert on both. Never `sleep` — use `expect.poll` and the locator waits. What a browser cannot show is the Tauri shell itself (the real menu bar, the OS save dialog, the global shortcut, window bounds); that is checked by hand on a virtual display. See the テスト section of `docs/DEVELOPMENT.md` for the details.
