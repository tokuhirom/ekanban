...
期限、タグ、検索、Markdown、通知、複数ユーザー、クラウド同期は MVP の対象外です。期限、タグ、検索、アーカイブ、Undo/Redo、非同期保存、複数ボードは先行して実装済みで、キーボード操作以降は引き続き拡張中です。詳しくは [実装ロードマップ](docs/ROADMAP.md) を参照してください。
