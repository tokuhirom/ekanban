...
- **カラム** — 追加、名前変更、削除、まとめてアーカイブ。「完了扱い」にしたカラムのカードは薄く沈み、期限の件数から外れます
- **絞り込み** — タイトルと説明の検索（全角半角・大文字小文字は同じものとして扱います）とタグ（カード上のタグを押します）。**カードは隠さず暗くします**
- **繰り返し** — 日次・平日・週次・月次・月末のタスクを、定義からカードにします。前のカードは受け持ちが終わった日に片付き、しばらく開かなくても出るのは 1 枚だけです
...
- クイックキャプチャのグローバルホットキー（ページの外まで届く割り当ては作れません）
- データベースやバックアップの場所をファイル管理で開くこと、データベースのコピー（ブラウザ版に SQLite のファイルがありません。**ボードの書き出しはダウンロードとして受け取れます**）
- メニューの割り当ての一部（`Ctrl+N` などはブラウザ自身が取ってしまいます。メニューから押せます）

消していない項目は、メニューに灰色で残しています。
