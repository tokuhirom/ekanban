...
| OS | 場所 |
| --- | --- |
| macOS | 画面上端のメニューバー（OS が描きます） |
| Linux / Windows | ウィンドウのいちばん上のメニューバー |

Linux と Windows では OS がメニューバーを描いてくれないので、ekanban が自分で出します。使い方は VS Code などと同じです。

- メニュー名をクリックすると開きます
- 開いている間は、ほかのメニュー名にマウスを載せるだけでそちらへ切り替わります
- `←` `→` でメニューを、`↑` `↓` で項目を移動し、`Enter` で実行します
- `Escape` で閉じます
- ショートカットのある項目は、右端にその組み合わせが出ます
...
保存は非同期です。失敗したときは、その操作の**前の状態に戻して**からダイアログで知らせます。画面に出ている内容とデータベースの中身が食い違ったままにはなりません。編集の途中だったときは、打った内容が編集パネルに戻るので、原因を直してから保存し直せます。
...
同じ理由で、**できない操作は理由を言う代わりにボタンを無効にします**。保存の途中と編集中はボード一覧と `＋` が、カードの無いカラムでは `…` の「アーカイブ」が押せなくなります。
...
### Linux で書き出しやコピーのダイアログが開かない

Linux では、保存先を選ぶダイアログを **xdg-desktop-portal** に頼んでいます。ポータル本体か、デスクトップ環境に合ったバックエンド（GNOME なら `xdg-desktop-portal-gnome`、KDE なら `xdg-desktop-portal-kde`、そのほかは `xdg-desktop-portal-gtk`）が入っていないと開きません。そのときは、何を入れればよいかを添えたエラーが出ます。

同じ理由で、「データベースの場所を開く」が何も起きないことと、テーマの「システムに合わせる」が常にライトになることがあります。テーマはメニューからライト / ダークを選べば効きます。
