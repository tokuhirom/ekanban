...
- プロジェクトの方針とビルド方法は [README](../README.md) にあります
...
- **中央上** がヘッダです。ボード名、保存の状態、検索、タグ、絞り込みが並びます
...
ヘッダのタグ名を押すと、そのタグの付いたカードだけが明るくなります。もう一度押すと解除です。

![タグで絞り込んだところ](images/filter-tag.png)

タグは「+ タグ」で追加し、各タグの「編集」「削除」で変えられます。色は自由に決められます。
...
| タグを追加 | `Cmd+Shift+T` | `Ctrl+Shift+T` |
| 保存 | `Cmd+S` | `Ctrl+S` |
