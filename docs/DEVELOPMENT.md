...
- **メニューを選ぶのは `e2e/harness.ts` の `chooseMenu(page, action)` です。** `window.ekanbanMenu` を直に呼びません。あの口を付けるのは React の effect（`src/ipc/browser.ts` の `onAppAction`）なので、ページが `load` を出したあとでも、盤面の `.column` が描かれたあとでも、まだ付いていないことがあります。`chooseMenu` は口が開くのを待ってから、**`?.` 無しで**呼びます——黙って捨てると、何も起きていない画面を待った先の `expect` が落ち、原因から遠いところで「遅い環境でだけ落ちるテスト」に見えます
