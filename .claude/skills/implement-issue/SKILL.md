...
- `docs/adr/README.md` — ADR の「いつ書くか」。判定はこの段階でする
...
- **ADR を書くか。** `docs/adr/README.md` の「いつ書くか」で判定する。要るなら、この設計コメントがそのまま下書きになる
...
- <要る / 要らない。要るなら番号と題、要らないならその理由を 1 行>
...
- **ADR が要ると判定したなら、このブランチで書く。** `docs/adr/` に連番で 1 ファイル。`docs/DESIGN.md` の該当する決まりごとから ADR へリンクし、**同じ内容を両方に書かない**（規則は `DESIGN.md`、経緯は ADR）。既にある ADR は書き換えない。覆すなら新しい ADR を足し、古いほうの「状態」に「置き換えられた（→ 番号）」と書き足す
...
`docs/DESIGN.md` の「変更の完了条件」を全部満たす。**10 番（重大な決定は ADR に残す）を忘れやすい。** 設計の段階で「要る」と判定したなら、ここで書けているか確かめる。
...
<足した ADR へのリンク。要らないと判定したならその理由を 1 行>
...
有効化できたら PR の URL を報告する。**マージの完了は待たない**が、**PR は見張り続ける**。

## 8. PR を見張る

作った PR は、マージされるか閉じられるまで自分の持ち物。作った直後に購読する。

```
subscribe_pr_activity(owner="tokuhirom", repo="ekanban", pullNumber=<PR番号>)
```

`gh` があるなら `gh pr checks <PR番号>` と `gh pr view <PR番号>` で同じことを見る。

見張っているあいだにすること:

- **CI が落ちた** → 落ちた検査を手元で再現してから直して push する。この枝の落ち度でないときは、その理由を PR にコメントする。黙って放置しない
- **conflict した** → `main` を枝へマージして解消し、生成物はツールで作り直してから push する。他人の枝の履歴は書き換えない
- **レビューが付いた** → 小さい指摘はその場で直して push し、直したスレッドは解決にする。大きい相談は提案を返して作者の判断を待つ

赤いまま、あるいは conflict したままの PR を「終わりました」と報告しない。
...
- 既にある ADR を書き換えない。覆すときは新しい ADR を足す
- 何にでも ADR を書かない。バグ修正・文言・リファクタリング・受け入れ条件どおりの実装には要らない
