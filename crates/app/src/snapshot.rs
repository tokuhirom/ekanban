...
pub struct Snapshot {
    ...
    /// 「今日が何日か」を知らないからです。
    ///
    /// **日付をまたぐと古くなります。** 開きっぱなしで日が変わると、この判定は
    /// 昨日のものになります。`today` を一緒に返すのはそのためで、webview は
    /// 手元の日付とこれを比べ、ずれていたら `snapshot` を呼び直します
    /// （`web/src/state/day.ts` と `web/src/state/board.ts`、#135）。比べるのに
    /// 手元の時計を使うだけで、期限の判定はここに残ります。
