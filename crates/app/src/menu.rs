//! メニューバーと、そこに付くキーの割り当て（`docs/TAURI-MIGRATION.md` §7）。
//!
//! **メニューは先に「データ」として組み、あとで Tauri のメニューに変換します。**
//! `tauri::menu::Menu` を作るにはアプリのハンドルが要り、そこだけで組むと構成を
//! 確かめるのに窓を開ける羽目になります。gpui 版の `menu.rs` が `Menu` の
//! 値を返して照合していたのと同じ形で、[`sections`] の返り値をテストが読みます。
//!
//! 押されたときの行き先は 2 つあります。
//!
//! - [`AppAction`] は webview へ流します（`app:action`、§3）。盤面に触るもの、
...
/// どの OS でも同じ「編集」メニュー。
///
/// **元に戻す・やり直すにアクセラレータを付けません**（§7）。付けると、説明欄を
